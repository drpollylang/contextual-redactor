    ...
    for i, img in enumerate(processed_imgs):
        ...
        if safe_cut is None:
            ...
            if white_cap <= 0:
                final_imgs.append(img)
                continue
            # move only white rows
            spill = img.crop((0, 0, W, white_cap))
            # append to prev bottom
            new_prev = Image.new("RGB", (pW, pH + white_cap), "white")
            new_prev.paste(prev_img, (0, 0))
            new_prev.paste(spill, (0, pH))
            # cut top of current
            new_curr = img.crop((0, white_cap, W, H))
...
if __name__ == "__main__":
    ...
    raster_reflow(
        ...
        output_pdf="Cleaned_FakeStackedEmailThread2.pdf",
        patterns=patterns,
        zoom=1.0,
