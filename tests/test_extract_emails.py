...
from redactor.utils import extract_emails, parse_thread
...
def test_extract_emails_wrapper():
    load_dotenv()
    ...
    # assert no_emails_test == 0, f"Expected 0 emails, found {no_emails_test}"
    ...
    print('-'*50 + '\nTesting stacked email thread with 3 emails:')
    stacked_test_doc = 'FakeStackedEmailThread.pdf'
    stacked_test = test_extract_emails(client, stacked_test_doc)
    assert stacked_test == 3, f"Expected 2 emails, found {stacked_test}"

if __name__ == "__main__":
    test_extract_emails_wrapper()